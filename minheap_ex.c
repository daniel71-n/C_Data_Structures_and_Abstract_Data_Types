...
/* The heap, as implemented here, relies on the structure of a binary tree.
   ...
   There two additional constraints to be complied with for a binary tree
   to be a heap:
   ...
   This is as opposed the explicit implementation of the heap, where use
   ...
   which is then 'sifted down'; inserting a new node is done at the open
   ...
   will invariably be found at the end of the traversing the queue,
   which means you ALWAYS have to carry out a complete traversal
   going over every single node. Knowing this, you could simply
   enqueue all the nodes, but jump at some calculated point deep
   into the queue, where you know the last level of the tree is
   supposed to start. This notwithstanding, this is an expensive 
   option.
   ...
   can efficiently find the slot where a new node needs to go in,
   you then have to 'sift up' or 'percolate up' this node, so as
   to find its correct position in the heap and abide by the 
   'heap property'.
   ...
   But you need not only its immediate parent, but all the nodes
   ...
   (Node positions are numbered breadth-first, from left to right).
    ...
    As can be seen above, this is all the way to the left down the tree.
    All the divisions above left no remainder. This is important: if the 
    division leaves no remained, then the number is even, and starting at
    root, the path goes the left. If there's a remainder of 1, the path
    is to the right, on the other hand.

    So the modulo operator will come in handy here, for tracing the path
    to the node starting at the root.
    Alternatively, you can use bitwise shifting. It will probably not lead
    to better performance though, as the compiler is most likely already
    ...
    ***********************************************
    ...
    slot for insertions, and then trace the path from root to this node. 
    This path is needed twice, in opposite directions: coming down from root
    to find the slot to insert the new node, and going back up the path
    during sift-up, after the node is inserted. 

    To avoid computing the path twice, a stack data structure is used. 
    stack1 holds the path down, stack2 the path back up, which is simply
    the path down, flipped upside down. 
    compute the path with bitwise shift:
    push0, push0, push1, for example.
    
    stack1                      stack2
    1  pop1 - go right - push1   0          
    0  pop0 - go left  - push0   0
    0  pop0 - go left  - push0   1
    ...
    has a pointer to the parent above it by popping this pointer off the
    stack. 
    ...
    the number of nodes and thus making determining the size a matter of
    looking it up rather than computing it every time.
    ...
    for that. It does have the advandate of being able to better
    visualize the heap structure (which is why heaps are often shown
    in tree form in pictorials) as well as being able to grow the 
...
static Stack *Heap_find_spot_P(Heap *the_heap){
    /* Returns NULL if the last level is filled, which 
       means it's safe to go one level deeper when inserting
       the new node, without compromising the heap property.
    */

    uint16_t slot = the_heap->count + 1;  // the empty slot is at position n+1, where n is the number of nodes in the heap
    Stack *way_down;   
    Stack_init(&way_down);
    ...
    StackItem *node = malloc(sizeof(StackItem));
    node->contents = the_heap->root;    // contents is a void pointer, so it can point to anything
    ...
    HeapNode *current = the_heap->root;
    // rightmost bit in 'slot' isn't needed when shifting, because we want not the exact
    // slot, but its parent
    slot >>= 1;
    
    while (slot > 1){  // > than 1 because the leftmost bit in heap_size needs to be disregarded
        temp = slot | 1;
        node = malloc(sizeof(StackItem));

        if (temp == 0){
            node->contents = current->left; 
        }else{
            node->contents = current->right;
        };
        slot >>= 1;
...
static void Heap_sift_up(Stack *path_to_root, HeapNode *current){
    HeapNode *parent = (Stack_pop(path_to_root))->contents;
    ...
    do{
        if (current->data < parent->data){
            ...
            current = parent;
            parent = (Stack_pop(path_to_root))->contents;
        }
    }while (Stack_count(path_to_root));
...
Heap *Heap_insert(Heap *the_heap, char the_value){
    // find slot where the value needs to be inserted
    // such that the shape property of the heap is respected
    Stack *path_to_slot = Heap_find_spot_P(the_heap);
    HeapNode *slot_parent = (Stack_peek(path_to_slot))->contents; // implcit conversion of void pointer to HeapNode pointer
    ...
    if (newnode){
        ...
        if (slot_parent->left == NULL){
            slot_parent->left = newnode;
        }
        else if (slot_parent->right == NULL){
            slot_parent->right = newnode;
        }
